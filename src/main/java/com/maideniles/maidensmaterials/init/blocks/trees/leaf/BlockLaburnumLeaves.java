...
public class BlockLaburnumLeaves extends BlockLeaves
{
    public BlockLaburnumLeaves(String name)
    {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (mainHandItem == MaidensItems.pruning_shears || playerIn.isPotionActive(MaidensMaterials.FLORAL_FORTUNE)) {
			if (!worldIn.isRemote) {
				...
				EntityItem itemblossoms = new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(),new ItemStack(MaidensItems.laburnum_blossoms, 1));
